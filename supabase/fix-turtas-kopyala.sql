-- Turtalar kategorisi — Supabase SQL Editor (blok blok Run)
...
-- BLOK 4
SELECT c.slug AS category, p.slug, p.name_tr, p.name_ru
FROM products p
LEFT JOIN categories c ON c.id = p.category_id
WHERE p.slug IN ('pie-apple-walnut-cinnamon', 'pie-snickers');
