...
import { getIcedCoffeeProducts } from '@/lib/coffee';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'icedCoffee' });
  return { title: t('title'), description: t('description') };
}

export default async function IcedCoffeePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations('icedCoffee');
  const { products, categories } = await getCatalogData();
  const icedProducts = getIcedCoffeeProducts(products, categories);
  ...
  return (
    <div className="pb-8">
      ...
      <div className="mt-6">
        {icedProducts.length === 0 ? (
          <p className="py-12 text-center text-sm text-muted">{t('empty')}</p>
        ) : (
          <CoffeeMenuBoard products={icedProducts} locale={locale as Locale} />
        )}
      </div>
    </div>
  );
